defmodule Latu.Client do
  ...
  **Killing the task instead is not the same thing, and it is worse.** A Latu execution is
  ...
  the same way onto the field of that name and read off the finished struct by `latu_ml`. The
  three streaming commands land on the fields named after their arms, read by
  `Latu.StreamingQuery`. Commands with nothing to say (writes, views) leave them all `nil` and
  run equally well through `execute/2`; this exists for the ones that answer, and for the
  metrics an observed write reports. PySpark's `execute_command`.
  """
