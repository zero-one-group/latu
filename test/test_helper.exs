...
Code.require_file("support/guides.exs", __DIR__)
...
# `:s3` is the same shape for a different reason: it needs a stack `docker compose up -d` does
# not start (dev/README.md).
ExUnit.start(exclude: [:integration, :golden, :streaming, :s3])
